...
/*
fn combine_dna(kitty1_dna: [u8; 16], kitty2_dna: [u8; 16], selector: [u8; 16]) -> [u8; 16] {
		let mut final_dna = kitty1_dna;
		
		for (i, (dna_2_element, r)) in kitty2_dna.as_ref().iter().zip(selector.as_ref().iter()).enumerate() {
        if r % 2 == 0 {
...
impl<T: Trait> Module<T> {
	...
	fn do_breed(sender: T::AccountId, kitty_id_1: T::KittyIndex, kitty_id_2: T::KittyIndex) -> dispatch::Result {
		...
		//let mut final_dna = kitty1_dna;
		//let final_dna = combine_dna(kitty1_dna, kitty2_dna, selector);
		//for (i, (dna_2_element, r)) in kitty2_dna.as_ref().iter().zip(selector.as_ref().iter()).enumerate() {
    //    if r % 2 == 0 {
    //        final_dna.as_mut()[i] = *dna_2_element;
    //    }
    //}
		...
		//Self::insert_kitty(sender, kitty_id, Kitty(final_dna));
		Ok(())
	}
}
