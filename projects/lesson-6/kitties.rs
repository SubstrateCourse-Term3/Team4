	...
	pub struct Module<T: Trait> for enum Call where origin: T::Origin {
		...
		/// Transfer a kitty to new owner
 		pub fn transfer(origin, to: T::AccountId, kitty_id: T::KittyIndex) {
			 ...
			 // ensure!(<OwnedKitties<T>>::exists((&sender, Some(kitty_id))), "Only owner can transfer kitty");
...
mod tests {
	...
	impl Trait for Test {
		...
		//type Randomness = randomness_collective_flip;
	...
	type OwnedKittiesTest = LinkedList<OwnedKitties<Test>, <Test as system::Trait>::AccountId, <Test as Trait>::KittyIndex>;
	...
	fn owned_kitties_can_append_values() {
		new_test_ext().execute_with(|| {
			...
			//<OwnedKitties<T>>
			assert_eq!(<OwnedKitties<Test>>::get(&(0, None)), Some(KittyLinkedItem {
 				prev: Some(1),
 				next: Some(1),
 			}));

  			assert_eq!(<OwnedKitties<Test>>::get(&(0, Some(1))), Some(KittyLinkedItem {
 				prev: None,
 				next: None,
 			}));
			...
			assert_eq!(<OwnedKitties<Test>>::get(&(0, None)), Some(KittyLinkedItem {
  			...
  			assert_eq!(<OwnedKitties<Test>>::get(&(0, Some(2))), Some(KittyLinkedItem {
 				prev: Some(1),
 				next: None,
 			}));
			...
			OwnedKittiesTest::append(&0, 3);
  			...
  			assert_eq!(<OwnedKitties<Test>>::get(&(0, Some(3))), Some(KittyLinkedItem {
 				prev: Some(2),
 				next: None,
 			}));
		});
 	...
 	fn owned_kitties_can_remove_values() {
		new_test_ext().execute_with(|| {
			...
			assert_eq!(<OwnedKitties<Test>>::get(&(0, None)), Some(KittyLinkedItem {
  			...
  			assert_eq!(<OwnedKitties<Test>>::get(&(0, Some(2))), None);
			...
			OwnedKittiesTest::remove(&0, 1);
  			...
  			assert_eq!(<OwnedKitties<Test>>::get(&(0, Some(1))), None);
