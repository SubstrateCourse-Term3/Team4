...
{
  int num=0xaabbccdd;
  char buffer[INT_BIT_NUM+1];
  buffer[INT_BIT_NUM]=0;
  makebinary(num,buffer);
  printf("%s",buffer);
}
*/
...
fn combine_dna(dna1: u8, dna2: u8, selector: u8) -> u8 {
	...
	//added by fisher
	//因为 selector是随机数据，那么直接 blake2_128（ dna1 + dna2 + selector）是不是简单些。
    //这里的变量 dna1，dna2， selector 的类型直接是  [u8; 16]， 而不是 u8
    
    /*
dna1 = 11110110, dna2= 1111010, selector=11010011
the index 0 ; bit = 128; choose is  0 
the index 1 ; bit = 128; choose is  2 
the index 2 ; bit = 128; choose is  0 
the index 3 ; bit = 128; choose is  8 
the index 4 ; bit = 128; choose is  16 
the index 5 ; bit = 128; choose is  32 
the index 6 ; bit = 128; choose is  64 
the index 7 ; bit = 128; choose is  0 
after combine_dna,dna is 02081632640 
    ...
    */
    ...
    println!( "after combine_dna,dna is {:b}" ,new_dna);
    //let rs = s.chars().rev();
...
impl<T: Trait> Module<T> {
	...
	fn do_breed(sender: T::AccountId, kitty_id_1: T::KittyIndex, kitty_id_2: T::KittyIndex) -> dispatch::Result {
		...
		for i in 0..kitty1_dna.len() {
            ...
            println!( "index{} ---data{ }", i, new_dna[i]);
		}
